...
if( isset( $_POST['mode'] ) || isset( $_GET['mode'] ) )
{
	...
	$mode = htmlspecialchars_wrapped($mode, ENT_COMPAT, 'utf-8');
}
else
{
	$mode = '';
...
if( isset ($_POST['deleteuser']) && $_POST['deleteuser'] && ( $userdata['user_id'] != $user_id ) && $new_user==0 )
		{
...
//-- fin mod : cache -------------------------------------------------------------------------------
		...
		$email = ( !empty($_POST['email']) ) ? trim(strip_tags(htmlspecialchars_wrapped( $_POST['email'], ENT_COMPAT, 'utf-8' ) )) : '';

		$password = ( !empty($_POST['password']) ) ? trim(strip_tags(htmlspecialchars_wrapped( $_POST['password'], ENT_COMPAT, 'utf-8' ) )) : '';
		$password_confirm = ( !empty($_POST['password_confirm']) ) ? trim(strip_tags(htmlspecialchars_wrapped( $_POST['password_confirm'], ENT_COMPAT, 'utf-8' ) )) : '';
		...
		if( isset( $_POST['avatargallery'] ) || isset( $_POST['submitavatar'] ) || isset( $_POST['cancelavatar'] ) )
		{
			...
			$occupation = htmlspecialchars_wrapped(stripslashes($occupation), ENT_COMPAT, 'utf-8');
			$interests = htmlspecialchars_wrapped(stripslashes($interests), ENT_COMPAT, 'utf-8');
			$signature = htmlspecialchars_wrapped(stripslashes($signature), ENT_COMPAT, 'utf-8');

			$user_lang = stripslashes($user_lang);
			$user_dateformat = htmlspecialchars_wrapped(stripslashes($user_dateformat), ENT_COMPAT, 'utf-8');
...
//-- fin mod : cache -------------------------------------------------------------------------------
		...
		else
		{
			...
			$username = htmlspecialchars_wrapped(stripslashes($username), ENT_COMPAT, 'utf-8');
			$email = stripslashes($email);
			$password = '';
			$password_confirm = '';
			...
			$user_dateformat = htmlspecialchars_wrapped($this_userdata['user_dateformat'], ENT_COMPAT, 'utf-8');
			$user_dateformat = htmlspecialchars_wrapped(stripslashes($user_dateformat), ENT_COMPAT, 'utf-8');
		}
	}
	else if( !isset( $_POST['submit'] ) && $mode != 'save' && !isset( $_POST['avatargallery'] ) && !isset( $_POST['submitavatar'] ) && !isset( $_POST['cancelavatar'] ) )
	{
		...
		$occupation = htmlspecialchars_wrapped($this_userdata['user_occ'], ENT_COMPAT, 'utf-8');
		$interests = htmlspecialchars_wrapped($this_userdata['user_interests'], ENT_COMPAT, 'utf-8');
	...
	if( isset($_POST['avatargallery']) && !$error )
	{
		if( !$error )
		{
			...
			{
			$category = htmlspecialchars_wrapped($_POST['avatarcategory'], ENT_COMPAT, 'utf-8');
			}
			else
			{
