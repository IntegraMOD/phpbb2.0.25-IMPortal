	...
	if ($level >= 0)
	{
		if ( ($tree['type'][$this_key] == POST_FORUM_URL) || (($tree['type'][$this_key] == POST_CAT_URL) && !$pull_down))
		{
			...
			if ( $data['tree.topic_last_post_id'] )
			{
				...
				$last_post .= '<a href="' . append_sid("viewtopic.$phpEx?"  . POST_POST_URL . '=' . $data['tree.topic_last_post_id']) . '#' . $data['tree.topic_last_post_id'] . '"><img src="' . $images['icon_latest_reply'] . '"  alt="' . $lang['View_latest_post'] . '" title="' . $lang['View_latest_post'] . '" /></a>';
			}
			...
			if ( $sub && ( !$pull_down || ( ($type == POST_FORUM_URL) && ($sub_forum > 0) ) ) && (intval($board_config['sub_level_links']) > 0) )
			{
				for ($j=0; $j < count($tree['sub'][$cur]); $j++) if ($tree['auth'][ $tree['sub'][$cur][$j] ]['auth_view'])
				{
					...
					if (intval($board_config['sub_level_links']) == 2)
					{
						...
						$wlast_post .= '<img src="' . $wfolder_image . '"  alt="' . $wfolder_alt . '" title="' . $wfolder_alt . '" style="vertical-align: middle" /></a>';
					}
