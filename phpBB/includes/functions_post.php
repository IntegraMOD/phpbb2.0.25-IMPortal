...
function prepare_post(&$mode, &$post_data, &$bbcode_on, &$html_on, &$smilies_on, &$error_msg, &$username, &$bbcode_uid, &$subject, &$message, &$poll_title, &$poll_options, &$poll_length)
{
	...
	if (!empty($subject))
	{
		$subject = htmlspecialchars_wrapped(trim($subject), ENT_COMPAT, 'utf-8');
	}
	...
	if ($mode == 'newtopic' || ($mode == 'editpost' && $post_data['first_post']))
	{
		...
		if (!empty($poll_title))
		{
			$poll_title = htmlspecialchars_wrapped(trim($poll_title), ENT_COMPAT, 'utf-8');
		}
		...
		if(!empty($poll_options))
		{
			...
			{
				$temp_option_text[intval($option_id)] = htmlspecialchars_wrapped($option_text, ENT_COMPAT, 'utf-8');
...
function submit_post($mode, &$post_data, &$message, &$meta, &$forum_id, &$topic_id, &$post_id, &$poll_id, &$topic_type, &$bbcode_on, &$html_on, &$smilies_on, &$attach_sig, &$bbcode_uid, $post_username, $post_subject, $post_message, $poll_title, &$poll_options, &$poll_length)
{
	...
	if (($mode == 'newtopic' || ($mode == 'editpost' && $post_data['edit_poll'])) && !empty($poll_title) && count($poll_options) >= 2)
	{
		...
		while (list($option_id, $option_text) = each($poll_options))
		{
			if (!empty($option_text))
			{
				$option_text = str_replace("\'", "''", htmlspecialchars_wrapped($option_text, ENT_COMPAT, 'utf-8'));
...
function clean_html($tag)
{
	...
	if (sizeof($matches))
	{
		...
		{
			return  htmlspecialchars_wrapped('</' . $matches[1] . '>', ENT_COMPAT, 'utf-8');
		}
	}
	...
	if (in_array(strtolower($tag[1]), $allowed_html_tags))
	{
		...
		if (!empty($tag[2]))
		{
			...
			for ($i = 0; $i < sizeof($test[0]); $i++)
			{
				...
				$attributes .= ' ' . $test[1][$i] . '=' . $test[2][$i] . str_replace(array('[', ']'), array('&#91;', '&#93;'), htmlspecialchars_wrapped($test[3][$i], ENT_COMPAT, 'utf-8')) . $test[2][$i];
			}
		}
		if (in_array(strtolower($tag[1]), $allowed_html_tags))
		{
		...
		{
			return htmlspecialchars_wrapped('<' . $tag[1] . $attributes . '>', ENT_COMPAT, 'utf-8');
		}
	}
	// Finally, this is not an allowed tag so strip all the attibutes and escape it
	else
	{
		return htmlspecialchars_wrapped('<' .   $tag[1] . '>', ENT_COMPAT, 'utf-8');
	}
}
?>