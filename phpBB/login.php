...
if( isset($_POST['login']) || isset($_GET['login']) || isset($_POST['logout']) || isset($_GET['logout']) )
{
		...
		if( $row = $db->sql_fetchrow($result) )
		{
			...
			else
			{
				...
				if( md5($password) == $row['user_password'] && $row['user_active'] )
				{
					...
					if( $session_id )
					{
					$url = ( !empty($_POST['redirect']) ) ? str_replace('&amp;', '&', htmlspecialchars_wrapped($_POST['redirect'], ENT_COMPAT, 'utf-8')) : "portal.$phpEx";
				...
				$redirect = ( !empty($_POST['redirect']) ) ? str_replace('&amp;', '&', htmlspecialchars_wrapped($_POST['redirect'], ENT_COMPAT, 'utf-8')) : '';
	...
	else if( ( isset($_GET['logout']) || isset($_POST['logout']) ) && $userdata['session_logged_in'] )
	{
		...
		{
		$url = (!empty($_POST['redirect'])) ? htmlspecialchars_wrapped($_POST['redirect'], ENT_COMPAT, 'utf-8') : htmlspecialchars_wrapped($_GET['redirect'], ENT_COMPAT, 'utf-8');
			$url = str_replace('&amp;', '&', $url);
			redirect(append_sid($url, true));
		}
		else
