	...
	case 'split':
		...
		if ($post_id_sql != '')
		{
			...
			if ($row = $db->sql_fetchrow($result))
			{
				...
				$post_subject = trim(htmlspecialchars_wrapped($_POST['subject'], ENT_COMPAT, 'utf-8'));
...
//-- fin mod : categories hierarchy ----------------------------------------------------------------
	...
	case 'ip':
		...
		$ip_this_post = ( $rdns_ip_num == $ip_this_post ) ? htmlspecialchars_wrapped(gethostbyaddr($ip_this_post), ENT_COMPAT, 'utf-8') : $ip_this_post;
		...
		if ( $row = $db->sql_fetchrow($result) )
		{
			...
			do
			{
				...
				$ip = ( $rdns_ip_num == $row['poster_ip'] || $rdns_ip_num == 'all') ? htmlspecialchars_wrapped(gethostbyaddr($ip), ENT_COMPAT, 'utf-8') : $ip;
