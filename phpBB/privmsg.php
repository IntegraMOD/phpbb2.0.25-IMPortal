...
if ( isset($_POST['folder']) || isset($_GET['folder']) )
{
	...
	$folder = htmlspecialchars_wrapped($folder, ENT_COMPAT, 'utf-8');
...
else if ( $mode == 'read' )
{
	...
	$template->assign_vars(array(
		...
		'U_REPLY_PM' => append_sid("privmsg.$phpEx?mode=reply&amp;" . POST_POST_URL . "=$privmsg_id"), 
		'U_EDIT' => append_sid("privmsg.$phpEx?mode=edit&amp;" . POST_POST_URL . "=$privmsg_id"),
		'U_QUOTE' => append_sid("privmsg.$phpEx?mode=quote&amp;" . POST_POST_URL . "=$privmsg_id"),
...
else if ( $submit || $refresh || $mode != '' )
{
	...
	if ( $submit )
	{
		...
		$privmsg_subject = trim(htmlspecialchars_wrapped($_POST['subject'], ENT_COMPAT, 'utf-8'));
	...
	else if ( $preview || $refresh || $error )
	{
		...
		//
		$to_username = (isset($_POST['username']) ) ? trim(htmlspecialchars_wrapped(stripslashes($_POST['username']), ENT_COMPAT, 'utf-8')) : '';

		$privmsg_subject = ( isset($_POST['subject']) ) ? trim(htmlspecialchars_wrapped(stripslashes($_POST['subject']), ENT_COMPAT, 'utf-8')) : '';
	...
	if ($error)
	{
		$privmsg_message = htmlspecialchars_wrapped($privmsg_message, ENT_COMPAT, 'utf-8');
		$template->set_filenames(array(
...
$template->assign_vars(array(
	...
	'U_POST_NEW_PM' => append_sid("privmsg.$phpEx?mode=post"))
);
