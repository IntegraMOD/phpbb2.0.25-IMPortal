...
{
	$post_order = (!empty($_POST['postorder'])) ? htmlspecialchars_wrapped($_POST['postorder'], ENT_COMPAT, 'utf-8') : htmlspecialchars_wrapped($_GET['postorder'], ENT_COMPAT, 'utf-8');
...
if (isset($_GET['highlight']))
{
	...
	$words = explode(' ', trim(htmlspecialchars_wrapped($_GET['highlight'], ENT_COMPAT, 'utf-8')));
...
if ( $is_auth['auth_mod'] )
{
	...
	$topic_mod .= '<a class="fa fa-trash" href="' . append_sid("modcp.$phpEx?" . POST_TOPIC_URL . "=$topic_id&amp;mode=delete&amp;p_sid=" . $userdata['priv_session_id']) . '" title="' . $lang['Delete_topic'] . '"><img src="' . $images['topic_mod_delete'] . '" alt="' . $lang['Delete_topic'] . '" /></a>&nbsp;';

	$topic_mod .= '<a class="fa fa-reply" href="' . append_sid("modcp.$phpEx?" . POST_TOPIC_URL . "=$topic_id&amp;mode=move&amp;p_sid=" . $userdata['priv_session_id']) . '" title="' . $lang['Move_topic'] . '"><img src="' . $images['topic_mod_move'] . '" alt="' . $lang['Move_topic'] . '" /></a>&nbsp;';

	$topic_mod .= ( $forum_topic_data['topic_status'] == TOPIC_UNLOCKED ) ? '<a class="fa fa-lock" href="' . append_sid("modcp.$phpEx?" . POST_TOPIC_URL . "=$topic_id&amp;mode=lock&amp;p_sid=" . $userdata['priv_session_id']) . '" title="' . $lang['Lock_topic'] . '"><img src="' . $images['topic_mod_lock'] . '" alt="' . $lang['Lock_topic'] . '" /></a>&nbsp;' : '<a class="fa fa-unlock" href="' . append_sid("modcp.$phpEx?" . POST_TOPIC_URL . "=$topic_id&amp;mode=unlock&amp;p_sid=" . $userdata['priv_session_id']) . '" title="' . $lang['Unlock_topic'] . '"><img src="' . $images['topic_mod_unlock'] . '" alt="' . $lang['Unlock_topic'] . '" /></a>&nbsp;';

	$topic_mod .= '<a class="fa fa-exchange" href="' . append_sid("modcp.$phpEx?" . POST_TOPIC_URL . "=$topic_id&amp;mode=split&amp;p_sid=" . $userdata['priv_session_id']) . '" title="' . $lang['Split_topic'] . '"><img src="' . $images['topic_mod_split'] . '" alt="' . $lang['Split_topic'] . '" /></a>&nbsp;';
}
...
if ( $can_watch_topic )
{
	if ( $is_watching_topic )
	{
		...
		$s_watching_topic_img = ( isset($images['topic_un_watch']) ) ? "<a href=\"viewtopic.$phpEx?" . POST_TOPIC_URL . "=$topic_id&amp;unwatch=topic&amp;start=$start&amp;sid=" . $userdata['session_id'] . '" title="' . $lang['Stop_watching_topic'] . '" class="fa fa-eye-slash" aria-hidden="true"><img src="' . $images['topic_un_watch'] . '" alt="' . $lang['Stop_watching_topic'] . '" /></a>' : '';
	}
	else
	{
		...
		$s_watching_topic_img = ( isset($images['Topic_watch']) ) ? "<a href=\"viewtopic.$phpEx?" . POST_TOPIC_URL . "=$topic_id&amp;watch=topic&amp;start=$start&amp;sid=" . $userdata['session_id'] . '" title="' . $lang['Start_watching_topic'] . '" class="fa fa-eye" aria-hidden="true"><img src="' . $images['Topic_watch'] . '" alt="' . $lang['Start_watching_topic'] . '" /></a>' : '';
	}
}
...
for($i = 0; $i < $total_posts; $i++)
{
	...
	if ( $poster_id != ANONYMOUS )
	{
		...
		$www_img = ( $postrow[$i]['user_website'] ) ? '<a class="fa fa-globe" aria-hidden="true" href="' . $postrow[$i]['user_website'] . '" target="_userwww" title="' . $lang['Visit_website'] . '"><img src="' . $images['icon_www'] . '" alt="' . $lang['Visit_website'] . '" /></a>' : '';
		$www = ( $postrow[$i]['user_website'] ) ? '<a href="' . $postrow[$i]['user_website'] . '" target="_userwww">' . $lang['Visit_website'] . '</a>' : '';

		$temp_url = append_sid("profile.$phpEx?mode=viewprofile&amp;" . POST_USERS_URL . "=$poster_id");
		$fb_img = ( $postrow[$i]['user_fb'] ) ? '<a class="fa fa-facebook-official" aria-hidden="true" href="https://www.facebook.com/' . $postrow[$i]['user_fb'] . '" target="blank" title="' . $lang['FB'] . '"><img src="' . $images['icon_fb'] . '" alt="' . $lang['FB'] . '" /></a>' : ''; 
		$fb = ( $postrow[$i]['user_fb'] ) ? '<a href="https://www.facebook.com/' . $postrow[$i]['user_fb'] . '" target="blank">' . $lang['FB'] . '</a>' : ''; 
		
		$ig_img = ( $postrow[$i]['user_ig'] ) ? '<a class="fa fa-instagram" aria-hidden="true" href="https://www.instagram.com/' . $postrow[$i]['user_ig'] . '" target="blank" title="' . $lang['IG'] . '"><img src="' . $images['icon_ig'] . '" alt="' . $lang['IG'] . '" /></a>' : ''; 
	...
	$quote_img = '<a class="fa fa-commenting-o" aria-hidden="true" href="' . $temp_url . '" title="' . $lang['Reply_with_quote'] . '"><img src="' . $images['icon_quote'] . '" alt="' . $lang['Reply_with_quote'] . '" /></a>';
	$quote = '<a href="' . $temp_url . '">' . $lang['Reply_with_quote'] . '</a>';

	$temp_url = append_sid("search.$phpEx?search_author=" . urlencode($postrow[$i]['username']) . "&amp;showresults=posts");
	$search_img = '<a class="fa fa-search" aria-hidden="true" href="' . $temp_url . '" title="' . sprintf($lang['Search_user_posts'], $postrow[$i]['username']) . '"><img src="' . $images['icon_search'] . '" alt="' . sprintf($lang['Search_user_posts'], $postrow[$i]['username']) . '" /></a>';
	$search = '<a href="' . $temp_url . '">' . sprintf($lang['Search_user_posts'], $postrow[$i]['username']) . '</a>';

	if ( ( $userdata['user_id'] == $poster_id && $is_auth['auth_edit'] ) || $is_auth['auth_mod'] )
	{
		$temp_url = append_sid("posting.$phpEx?mode=editpost&amp;" . POST_POST_URL . "=" . $postrow[$i]['post_id']);
		$edit_img = '<a class="fa fa-pencil-square-o" aria-hidden="true" href="' . $temp_url . '" title="' . $lang['Edit_delete_post'] . '"><img src="' . $images['icon_edit'] . '" alt="' . $lang['Edit_delete_post'] . '" /></a>';
	...
	if ( $is_auth['auth_mod'] )
	{
		...
		$ip_img = '<a class="fa fa-info" aria-hidden="true" href="' . $temp_url . '" title="' . $lang['View_IP'] . '"><img src="' . $images['icon_ip'] . '" alt="' . $lang['View_IP'] . '" /></a>';
		$ip = '<a href="' . $temp_url . '">' . $lang['View_IP'] . '</a>';

		$temp_url = "posting.$phpEx?mode=delete&amp;" . POST_POST_URL . "=" . $postrow[$i]['post_id'] . "&amp;p_sid=" . $userdata['priv_session_id'];
		$delpost_img = '<a class="fa fa-trash" aria-hidden="true" href="' . $temp_url . '" title="' . $lang['Delete_post'] . '"><img src="' . $images['icon_delpost'] . '" alt="' . $lang['Delete_post'] . '" /></a>';
